...
string Cat::getFeedSchedule() const 
{
    if (age < 1) 
    {
        return "Кормить 3-4 раза в день кормом для котят";
    }
    else 
    {
...
{
    string protocol = "Стандартный протокол лечения кошек:\n";
...
void Cat::printInfo() const 
{
    ...
    if (isLitterTrained) 
    {
        cout << "Приучен к лотку: Да" << endl;
    }
    else 
    {
        cout << "Приучен к лотку: Нет" << endl;
    }
}
