...
void AnimalHospital::removeAnimal(int id) {
    auto it = remove_if(animals.begin(), animals.end(),
        [id](const shared_ptr<Animal>& a) { return a->getId() == id; });
...
void AnimalHospital::createAppointment(shared_ptr<Animal> animal, const string& vet, const string& date, const string& time, const string& reason) {
    schedule.push_back(make_unique<Appointment>(animal, vet, date, time, reason));
}
